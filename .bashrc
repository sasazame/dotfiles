...
# erasedups = remove all previous occurrences of the command
...
# Custom function to search history with directory
hist() {
    if [ -f ~/.bash_history_full ]; then
        grep -i "${1:-}" ~/.bash_history_full | tail -n 50
    else
        history | grep -i "${1:-}"
    fi
}

# Show history for current directory
histhere() {
    if [ -f ~/.bash_history_full ]; then
        grep " $(pwd) " ~/.bash_history_full | tail -n 30
    else
        echo "Full history log not found"
    fi
}
